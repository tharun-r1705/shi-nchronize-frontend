...
Provide ONLY the JSON response, no additional text.`;
    ...
    return res.status(500).json({ 
      ...
      error: error.message,
      suggestion: 'The AI service may be experiencing high demand. Please wait a moment and try again.'
    });
  }
});
...
const extractResumeText = asyncHandler(async (req, res) => {
  ...
  try {
    // pdf-parse returns a promise that resolves with the data
    // Use pdfParse.pdf (the actual parser function) or pdfParse.PDFParse
...
const importLinkedInProfile = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const filledFields = Object.entries(profile)
      .filter(([key, value]) => ['firstName', 'lastName', 'headline', 'summary', 'location', 'linkedinUrl', 'skills'].includes(key) && value && (Array.isArray(value) ? value.length : true))
      .map(([key]) => key);
